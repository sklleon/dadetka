...
def головы_ленты(limit=50):
    ...
    """
    try:
        медиа = get(f"{IG_USER}/media", fields="caption,timestamp,media_product_type",
                    limit=limit).get("data", [])
    except MetaError:
        return None
    return [((m.get("caption") or "").strip()[:60], (m.get("timestamp") or "")[:10],
             род_поста(m.get("media_product_type")))
            for m in медиа]


def род_поста(тип: str | None) -> str:
    """Тип площадки → род нашей очереди: «ролик» или «пост».

    Род нужен ровно для одного: решить, занят ли сегодняшний день. Карусель,
    обложка и сторис для этого одно и то же — они конкурируют за ленту; ролик
    живёт своей жизнью.
    """
    return "ролик" if (тип or "").upper() == "REELS" else "пост"


def род_пункта(пункт: dict) -> str:
    """Запись очереди → род. Зеркало `род_поста` со стороны нашей очереди."""
    return "ролик" if пункт.get("format") == "reels" else "пост"


def решить(очередь, лента, сегодня, род=None):
    ...
    # ⛔ 25.09.2026 владелец снял правило «второй пост за день не шлю»: «нет таких
    # ограничений». Он прав дважды. Ограничение было НАШЕ, а не площадки — Meta
    # принимает до 50 публикаций в сутки. И оно не различало рода: ролик не мог
    # выйти в день карусели, хотя «это совсем другой контент» (его слова).
    #
    # 📌 Что осталось вместо него и почему не пустота: один каждого рода в день.
    # Снять счёт совсем значит залить всю очередь за один запуск — ровно то, от
    # чего правило и ставили в сентябре, когда хвост копился двенадцатью
    # записями. Число меняется здесь одной строкой.
    лента = [(з + ("пост",))[:3] for з in лента]        # старые пары — это посты
    занято = {р for _г, д, р in лента if д == сегодня.isoformat()}
    головы = {г for г, _д, _р in лента}
    пропущено = None
    for п in очередь["items"]:
        ...
        р = род_пункта(п)
        if род is not None and р != род:
            continue                    # спрашивали другой род
        if р in занято:
            пропущено = р
            continue                    # этого рода сегодня уже был
        return п, f"следующий: {п['key']} ({п['format']}, план {п['date']})"
    if пропущено:
        return None, f"{пропущено} сегодня уже выходил — следующий завтра"
...
def main():
    ...
    if args.key:
        ...
        # По одному каждого рода за запуск: пост и ролик за день, а не вся
        # очередь подряд. Решение владельца 25.09.2026 — ролик и карусель разный
        # контент и в один день друг другу не мешают.
        q = load_queue()
        лента = головы_ленты()
        сегодня = datetime.now(MSK).date()
        items = []
        for р in ("пост", "ролик"):
            п, почему = решить(q, лента, сегодня, род=р)
            print(f"📅 {р}: {почему}")
            if п:
                items.append(п)
    else:
